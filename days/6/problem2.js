...
const times = allInputs[0].split(':')[1].replace(/\s+/g, '').split(' ');
const distances = allInputs[1].split(':')[1].replace(/\s+/g, '').split(' ');
...
for (let i = 0; i < times.length; i++) {
  ...
  while (holdTime < raceTime + 1) {
    ...
    // console.log(
    //   `holdTime: ${holdTime}, record: ${raceRecord}, distance: ${distance}, beatRecord: ${beatRecord}, possibleWinningTimes: ${possibleWinningTimes}`,
    // );
